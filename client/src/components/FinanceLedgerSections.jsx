...
function SupplierForm({ catalogue, onCreated, onCancel }) {
  ...
  return <form onSubmit={submit} className="space-y-4 rounded-2xl border border-teal-200 bg-teal-50/40 p-4">
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ...
      <label className="grid gap-1 text-xs font-bold">Linked stock shipment<select value={form.shipment_id} onChange={(event) => setForm({ ...form, shipment_id: event.target.value })} className={formControlClass}><option value="">Not linked</option>{(catalogue?.shipments || []).map((shipment) => <option key={shipment.id} value={shipment.id}>#{shipment.id} · {shipment.supplier || "Supplier"} · {shipment.delivery_note || "No delivery note"}</option>)}</select></label>
