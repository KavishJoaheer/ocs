...
function TimelineEventCard({ event, reportUrlBuilder }) {
  ...
  return (
    <div className="relative flex gap-5 max-md:gap-3">
      ...
      <article
        className={`min-w-0 flex-1 rounded-xl border border-[rgba(26,160,140,0.12)] bg-white px-6 py-5 lg:border-brand-teal/20 ${MOBILE_CARD}`}
      >
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[15px] font-bold text-[#1a5c52] max-md:text-[14px] lg:text-brand-dark-grey">
              {dayjs(event.date).format("D MMMM YYYY")}
            </p>
            <div className="mt-1 flex items-center gap-2">
              {isConsultation ? (
                <Stethoscope className="size-3.5 shrink-0 text-[#2d8f98] lg:text-brand-teal" strokeWidth={1.75} />
              ) : (
                <FileText className="size-3.5 shrink-0 text-[#2d8f98] lg:text-brand-teal" strokeWidth={1.75} />
              )}
              <p className="truncate text-sm font-medium text-[#5b7f8a] max-md:text-[14px] lg:text-brand-cool-grey">
                {subtitle}
              </p>
            </div>
          </div>
          <span className="shrink-0 rounded-full bg-[rgba(26,160,140,0.08)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#2d8f98] max-md:text-[10px] lg:bg-brand-teal/10 lg:text-brand-dark-grey">
            {isConsultation ? "Visit" : "Report"}
          </span>
        </div>

        {isConsultation && event.title ? (
          <span className="mt-3 inline-flex rounded-[20px] bg-[rgba(26,160,140,0.1)] px-4 py-1 text-[13px] leading-[1.5] text-[#2d8f98] max-md:text-[14px] lg:bg-brand-teal/10 lg:text-brand-dark-grey">
            {event.title}
          </span>
        ) : null}

        {!isConsultation ? (
          <h3 className="mt-3 text-base font-semibold text-[#22485b] max-md:text-[14px] lg:text-brand-dark-grey">{event.title}</h3>
        ) : null}

        {shouldShowPlainSummary(isConsultation ? event.title : null, event.detail) ? (
          <p className="mt-2 text-[13px] font-light leading-[1.5] text-[#5b7f8a] max-md:text-[14px] lg:text-brand-cool-grey">
            {event.detail}
          </p>
        ) : null}
          ...
          <div className="mt-4 space-y-2">
            {event.reports.map((report) => (
              <div key={report.id} className="flex items-center justify-between gap-3">
                <span className="truncate text-sm text-[#1a5c52] max-md:text-[14px] lg:text-brand-dark-grey">{report.name}</span>
                <a
